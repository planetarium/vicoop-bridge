...
try {
  ...
  // Smoke-only Docker stdio probe; production execution uses the host broker.
  const child = spawn('docker', ['exec', '-i', '-w', '/tmp', '-e',
    'VB_SMOKE=literal $value; with spaces', container, 'sh', '-c', 'IFS= read -r line; printf "%s|%s|%s" "$PWD" "$VB_SMOKE" "$line"; printf "stderr" >&2'], {
