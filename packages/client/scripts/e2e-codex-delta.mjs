// E2E for the stateful-context delta path (issue #410) on the codex backend.
//
...
function makeTask(withContext) {
  ...
  return {
    type: 'task.assign',
    taskId: `e2e-codex-delta-${withContext ? 'with' : 'without'}-${t0}`,
    contextId: `e2e-codex-delta-ctx-${t0}`,
    message: {
      role: 'user',
      messageId: `m-${t0}`,
...
async function runArm(withContext) {
  const backend = createCodexBackend({ sandboxMode: 'read-only' });
  const frames = [];
...
console.log(`\n[e2e] ${failed ? 'RESULT: FAIL' : 'RESULT: PASS — #410 codex delta contextHistory verified end-to-end'}`);
