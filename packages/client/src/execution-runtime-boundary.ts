...
export function assertBrokerContainer(
  raw: string,
  kind: string,
  expectedName: string,
  ...
  try {
    ...
    throw new Error('Cannot inspect runtime authentication boundary');
  }
  const invalid = (reason = 'unexpected or missing runtime mount') => {
    throw new Error(
      `${kind} runtime requires host-broker migration (${reason}); see docs/${kind}-auth-broker.md. Existing credentials and volumes have not been deleted.`,
    );
  };
  // Reasons are fixed host-side strings; rejected inspect values stay private.
  const requireBoundary = (condition: unknown, reason: string) => {
  ...
  requireBoundary(
    ['node', '1000:1000', '1000'].includes(config?.User),
    'workload must use the unprivileged runtime user',
  );
  requireBoundary(
    !host?.Privileged &&
      ['default', 'bridge'].includes(host?.NetworkMode) &&
      !host?.PidMode &&
      ['', 'private', undefined].includes(host?.IpcMode) &&
      !host?.UsernsMode,
    'privileged or shared runtime namespaces are unsupported',
  );
  requireBoundary(
    !host?.DeviceRequests?.length &&
      !host?.Devices?.length &&
      !host?.VolumesFrom?.length,
    'device access and inherited volumes are unsupported',
  );
  requireBoundary(
    host?.CapAdd?.some(
      (value: string) => value.replace(/^CAP_/, '') === 'NET_ADMIN',
    ),
    'NET_ADMIN is required for the runtime firewall',
  );
  requireBoundary(
    !host?.CapAdd?.some(
      (value: string) =>
        !['NET_ADMIN', 'NET_RAW'].includes(value.replace(/^CAP_/, '')),
    ),
    'unexpected runtime capabilities',
  );
  requireBoundary(
    !host?.SecurityOpt?.some((value: string) =>
      /^(seccomp|apparmor)[=:]unconfined$/.test(value),
    ),
    'unconfined security profiles are unsupported',
  );
  requireBoundary(
  ...
  if (env.some((v) => PROVIDER_ENV_PATTERN.test(v.split('=', 1)[0])))
    invalid('provider environment variables are unsupported');
  if (
    !env.includes(
      `${
        kind === 'claude' ? 'CLAUDE_CONFIG_DIR' : 'CODEX_HOME'
      }=/data/sessions/${kind}/config`,
    )
  )
    invalid('runtime configuration directory mismatch');
  ...
  for (const mount of c.Mounts ?? []) {
    ...
    if (
      mount.Type === 'tmpfs' &&
      ['/tmp', `/data/creds/${kind}`].includes(mount.Destination)
    )
      continue;
    if (
      mount.Type === 'volume' &&
      mount.Destination === `/data/agents/${kind}` &&
      mount.Name === `vicoop-agents-${expectedName}`
    )
      continue;
    if (
      mount.Type === 'volume' &&
      mount.Destination === `/data/sessions/${kind}` &&
      mount.Name === `vicoop-sessions-${expectedName}`
    )
      continue;
    if (mount.Type === 'bind' && mount.Destination === '/workspace') {
      ...
      if (
        ...
        canonicalPath(mount.Source) !== canonicalPath(expectedWorkspace)
      ) {
        throw new Error(
          'Runtime workspace differs from the requested working directory; use the original workspace or initialize another runtime',
        );
      }
      workspaceFound = true;
      continue;
  ...
  if (expectedWorkspace !== undefined && !workspaceFound)
    throw new Error(
      'Runtime has no workspace mount for the requested working directory',
    );
}
...
export function brokerFirewallScript(): string {
  ...
  const blocked = [
    ...new Set([
      ...
      '224.0.0.0/4',
      '240.0.0.0/4',
      ...hostIPs.filter((v) => !v.startsWith('127.')),
    ]),
  ];
  return `set -eu
...
${blocked
  .map((ip) => `iptables -w -A VICOOP_BROKER -d ${ip} -j REJECT`)
  .join('\n')}
...
function canonicalPath(value: string): string {
  ...
  try {
    ...
    if (
      ...
      throw new Error('Cannot validate workspace path');
    const parent = dirname(absolute);
    if (parent === absolute) return absolute;
    return join(canonicalPath(parent), basename(absolute));
  }
}
export function assertBrokerWorkspace(
  source: string,
  ...
  for (const store of [
    ...
    if (
      contains(workspace, credentialDir) ||
      contains(credentialDir, workspace)
    )
      throw new Error(
        'Workspace overlaps a host credential store; select a separate project directory',
      );
  }
}
