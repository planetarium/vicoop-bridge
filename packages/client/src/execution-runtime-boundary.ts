...
export function assertBrokerContainer(raw: string, kind: string): void {
  let c;
  ...
  if (c.Config?.Labels?.[`vicoop.${kind}-auth`] !== 'stdio-v1' || !['node', '1000:1000', '1000'].includes(c.Config?.User) ||
  ...
  for (const mount of c.Mounts ?? []) {
    ...
    if (mount.Type === 'volume' && mount.Destination === `/data/agents/${kind}` && mount.Name === `vicoop-agents-${c.Config.Labels['vicoop.name']}`) continue;
    if (mount.Type === 'volume' && mount.Destination === `/data/sessions/${kind}` && mount.Name === `vicoop-sessions-${c.Config.Labels['vicoop.name']}`) continue;
