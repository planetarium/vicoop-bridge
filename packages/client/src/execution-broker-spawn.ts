...
import type { createExecutionAuthBroker } from './execution-auth-broker.js';
import { EXECUTION_BROKER_RELAY } from './execution-broker-relay.js';
import { EXECUTION_BROKER_SUPERVISOR } from './execution-broker-supervisor.js';
...
export function createExecutionBrokerSpawn(container: string, opts: {
  backend: 'claude' | 'codex';
  createBroker: () => ReturnType<typeof createExecutionAuthBroker>;
  ttlMs?: number;
  ...
  const spawn: SpawnFn = (command, args, options) => {
    if (stopped) throw new Error('Execution authentication transport is stopped');
    ...
    for (const key of Object.keys(env)) {
      if (!['ENABLE_PROMPT_CACHING_1H', 'CLAUDE_CODE_MAX_OUTPUT_TOKENS', 'MAX_THINKING_TOKENS', 'CLAUDE_CODE_EFFORT_LEVEL', 'CLAUDE_CODE_DISABLE_ADAPTIVE_THINKING'].includes(key)) {
        throw new Error(`Unsupported broker container spawn environment: ${key}`);
      }
    }
    const promptFiles: Array<{argIndex:number; content:Buffer}> = [];
    let stagedBytes = 0;
    if(opts.backend==='codex' && opts.codexCatalog) {
      const content=Buffer.from(opts.codexCatalog);
      if(content.length>2*1024*1024)throw new Error('Codex model catalog exceeds staging limit');
      promptFiles.push({argIndex:-1,content});stagedBytes=content.length;
    }
    ...
    relay.on('close', (code) => {
      ...
      const cleanupFinished = (confirmed: boolean) => {
        ...
        if (!confirmed) console.error('Runtime cleanup could not be confirmed; new executions are disabled. Restore Docker access and stop the runtime before restarting.');
        finish(1, null);
      };
    ...
    void (async () => {
      ...
      send({ t: 'start', backend: opts.backend, command, args, cwd: options.cwd, env, token: broker.token,
