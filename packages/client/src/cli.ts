...
// Compact one-line usage stamped at the top of every error message. The
// grouped, descriptive form lives in `DAEMON_HELP` below and is reserved
// for `--help` / `help` invocations.
const DAEMON_USAGE =
  'vicoop-client [--server <ws://...>] --token <t> --agentId <id> [--backend <name>] [flags…]';

// Grouped help text printed for `vicoop-client --help` / `vicoop-client help`
// — issue #189 AC#3 ("`--help` lists every accepted flag, grouped sensibly").
// Hand-rolled rather than auto-generated by optique because tests assert on
// exact wording (`usage:` prefix, the per-subcommand `usage()` strings) and
// the prose contextualisation per group is worth the maintenance.
const DAEMON_HELP = [
  `usage: ${DAEMON_USAGE}`,
  '',
  ...
  '',
  'Connection:',
  `  --server <ws://...>     Bridge WS URL. Default ${DEFAULT_BRIDGE_URL}`,
  '                          (public bridge); set only when self-hosting.',
  '  --card <path>           Agent card JSON override (defaults to server-published card).',
  '  --config <path>         Explicit config.json overlaid on the canonical file.',
  '',
  'Backend selection:',
  '  --backend <name>        echo | openclaw | claude | codex. Default: echo.',
  '',
  'Backend-specific (Claude):',
  '  --claude-cwd <path>            Working directory for the spawned `claude`.',
  '  --claude-settings-file <path>  Path to a JSON file used as Claude `--settings`.',
  '',
  'Backend-specific (Codex):',
  '  --codex-cwd <path>      Working directory for the spawned `codex`.',
  '  --codex-sandbox <mode>  read-only | workspace-write | danger-full-access.',
  '',
  'Backend-specific (OpenClaw):',
  '  --openclaw-gateway <url>             Gateway WS URL (default ws://127.0.0.1:18789).',
  '  --openclaw-gateway-token <t>         Auth token if your gateway requires one.',
  '  --openclaw-agent <name>              Primary agent name. Default: main.',
