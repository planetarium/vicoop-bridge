...
test('fixed storage retains identity, rejects legacy adoption and changes, terminates aborted helpers', async t => {
  ...
  const storage = new CallerFixedStorage(options, store, run);
  ...
  await assert.rejects(new CallerFixedStorage({ ...options, storageMiB: 128 }, store, run).record(id), /configuration changed/);
  await assert.rejects(new CallerFixedStorage({ ...options, fixedImageStorage: { ...options.fixedImageStorage!, poolVolume: 'other' } }, store, run).record(id), /configuration changed/);
