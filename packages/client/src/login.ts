...
function writeEnvFile(path: string, success: OwnerSessionSuccess, bridgeUrl: string): void {
  // `export` so that `. vicoop-owner.env` propagates these to admin
  // subcommands (`add-caller`, `list-agents`, ...) run in the same shell.
  atomicWriteFile(path, [
    `# vicoop-client env (generated by 'vicoop-client login')`,
    `export VICOOP_BRIDGE=${bridgeUrl.replace(/\/$/, '')}`,
    `export VICOOP_OWNER_TOKEN=${success.access_token}`,
    '',
  ].join('\n'), 0o600);
}
