...
function writeClientEnvFile(path: string, success: ClientRegisterSuccess, bridgeUrl: string): void {
  ...
  atomicWriteFile(path, [
    ...
    `export SERVER_URL=${bridgeUrl.replace(/^http(s?):\/\//, (_m, s) => (s === 's' ? 'wss://' : 'ws://'))}`,
    `export SERVER_TOKEN=${success.client_token}`,
    `export AGENT_ID=${success.allowed_agent_ids[0] ?? ''}`,
    '',
  ].join('\n'), 0o600);
}
