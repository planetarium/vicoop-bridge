...
test('caller runtime rejects anonymous/delegated identity before binding or payment work', async () => {
  for (const persistenceFails of [false, true]) for (const metadata of [{}, { _principalId: 'apikey:a', _actorId: 'actor' }, { _principalId: 'apikey:a', _authorizationKey: 'grant' }]) {
    const { ws, sent } = makeWsCapture();
    const registry = new Registry();
    ...
    const store = noopTaskStore();
    if (persistenceFails) store.updateTask = async () => { throw new Error('temporary task store outage'); };
    const executor = new WSForwardingExecutor('agent', registry, store);
