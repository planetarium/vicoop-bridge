...
test('maps v0.3 and v1 JSON-RPC methods to the narrow federation scopes', () => {
  assert.deepEqual(
    parseFederatedJsonRpcOperation({ method: 'message/send', params: { message: { taskId: 't-1' } } }),
    { scope: OAUTH_FEDERATION_SCOPE_MESSAGE_SEND, kind: 'message', taskId: 't-1' },
  );
  assert.deepEqual(
    parseFederatedJsonRpcOperation({ method: 'GetTask', params: { id: 't-2' } }),
    { scope: OAUTH_FEDERATION_SCOPE_TASK_READ, kind: 'task', taskId: 't-2' },
  );
  assert.deepEqual(
    parseFederatedJsonRpcOperation({ method: 'SubscribeToTask', params: { taskId: 't-3' } }),
    { scope: OAUTH_FEDERATION_SCOPE_TASK_RESUBSCRIBE, kind: 'task', taskId: 't-3' },
  );
});

test('maps HTTP+JSON task paths without confusing task ids and actions', () => {
  assert.deepEqual(
    parseFederatedHttpJsonOperation('GET', '/agents/a/v1/tasks/t%2F1', undefined),
    { scope: OAUTH_FEDERATION_SCOPE_TASK_READ, kind: 'task', taskId: 't/1' },
  );
  assert.deepEqual(
  ...
  assert.deepEqual(
    parseFederatedHttpJsonOperation('GET', '/agents/a/v1/tasks', undefined),
    { scope: OAUTH_FEDERATION_SCOPE_TASK_READ, kind: 'task-list' },
  );
});
...
test('unknown methods stay closed for federated bearer enforcement', () => {
  ...
  assert.equal(parseFederatedHttpJsonOperation('PATCH', '/agents/a/v1/tasks/t-1', undefined), undefined);
});
...
test('continuation tokens retain principal, actor, grant, and task binding', async () => {
  ...
  assert.deepEqual(await authorizeFederatedOperation(sql, 'agent-1', federatedCaller(), {
  ...
  assert.deepEqual(await authorizeFederatedOperation(sql, 'agent-1', taskOnly, {
...
test('message-path task scopes still require the task principal to match', async () => {
  const result = await authorizeFederatedOperation(authorizationSql({
    actorId: 'did:web:connector.example',
    principalId: 'slack:T123/OTHER',
    authorizationKey: 'federated-key-1',
    active: true,
  }), 'agent-1', federatedCaller(), {
    scope: OAUTH_FEDERATION_SCOPE_TASK_READ,
    kind: 'task',
    taskId: 'task-1',
  });
...
test('task continuity fails closed on actor mismatch or removed exact grant', async () => {
  ...
  assert.deepEqual(
    await authorizeFederatedOperation(authorizationSql({
      actorId: 'did:web:other.example',
      principalId: caller.principalId,
      authorizationKey: 'federated-key-1',
      active: true,
    }), 'agent-1', caller, operation),
    { ok: false, reason: 'task_actor_mismatch' },
  );
  assert.deepEqual(
    await authorizeFederatedOperation(authorizationSql({
      actorId: caller.federation!.actorId,
      principalId: caller.principalId,
      authorizationKey: 'federated-key-1',
      active: false,
    }), 'agent-1', caller, operation),
...
test('a federated task remains protected after the agent becomes public', async () => {
  ...
  assert.deepEqual(result, { ok: false, reason: 'federated_task_requires_token' });
  assert.deepEqual(await authorizeFederatedOperation(authorizationSql({
    actorId: 'did:web:connector.example',
    principalId: 'slack:T123/U456',
    authorizationKey: 'federated-key-1',
    active: false,
